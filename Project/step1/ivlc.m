...
for i=1 : length(vlcStream)

    if length(vlcStream(i:end))<=2
        if strcmp(vlcStream(i:end), '10')
            return
        else
            disp('You propably forgot to insert EOB')
        end
    end
    ...
    if escaped==0
            if any(strcmp(d15b(:),sequence))
                ...
                if strcmp(sequence, '000001') % Here I am about to jump to search in another table
                    escaped=1;
                    sequence='';
                    disp('I escaped')
                else
...
%% The following code can be used if the vlcStream is saved in an cell array, where in each cell there is one runLength code into vlc stream. Not used in our case, since the vlcStream is in one string
